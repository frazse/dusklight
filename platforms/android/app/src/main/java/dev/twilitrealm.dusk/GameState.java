...
public class GameState {
    ...
    public GameState(int[] i, float[] f, String stageName, String itemTitle, String itemDesc, float[] lines, float[] icons, float[] doors, int batteryLevel, boolean isCharging) {
        ...
        this.buttonAText = (inMenu || (visMask & 1) != 0) ? getActionLabel(idA, isSwimming, isRiding, isFlying, transform, false, inMenu) : "";
        this.buttonBText = (inMenu || (visMask & 2) != 0) ? getActionLabel(idB, isSwimming, isRiding, isFlying, transform, false, inMenu) : "";
        
        String zLabel = getActionLabel(idZ, isSwimming, isRiding, isFlying, transform, true, inMenu);
        if (zLabel.isEmpty() && !inMenu && windowStatus == 0) {
            zLabel = "Midna";
        }
        this.buttonZText = (inMenu || (visMask & 4) != 0 || !zLabel.isEmpty()) ? zLabel : "";

        String lLabel = getActionLabel(idL, isSwimming, isRiding, isFlying, transform, false, inMenu);
        ...
        this.buttonRText = (inMenu || (visMask & 8) != 0) ? getActionLabel(idR, isSwimming, isRiding, isFlying, transform, false, inMenu) : "";
        this.buttonXText = (inMenu || (visMask & 16) != 0) ? getActionLabel(idX, isSwimming, isRiding, isFlying, transform, false, inMenu) : "";
        this.buttonYText = (inMenu || (visMask & 32) != 0) ? getActionLabel(idY, isSwimming, isRiding, isFlying, transform, false, inMenu) : "";

        this.dPadUpText    = getActionLabel(i[35], isSwimming, isRiding, isFlying, transform, false, inMenu);
        this.dPadDownText  = getActionLabel(i[36], isSwimming, isRiding, isFlying, transform, false, inMenu);
        this.dPadLeftText  = getActionLabel(i[37], isSwimming, isRiding, isFlying, transform, false, inMenu);
        this.dPadRightText = getActionLabel(i[38], isSwimming, isRiding, isFlying, transform, false, inMenu);
    ...
    private String getActionLabel(int id, boolean isSwimming, boolean isRiding, boolean isFlying, int wolfForm, boolean isZ, boolean inMenu) {
        if (id == 0) return "";
        
        // Priority 1: Menu specific mappings (Truth Mirror)
        if (inMenu) {
            switch(id) {
                ...
                case 0x4B: return "Skip"; // Menu Skip
            }
        }

        // Priority 2: Wolf specific mappings (to avoid collisions)
        if (wolfForm == 1) {
            switch(id) {
        ...
        // Priority 3: Human / Item mappings
        switch (id) {
            ...
            case 0x0D: return "Bomb"; // COLLISION FIX: Item ID 0x0D is Bomb 30
            ...
            case 0x43: return "Bow"; // COLLISION FIX: Item ID 0x43 is Bow
            ...
            case 0x4B: return "Slingshot"; // COLLISION FIX: Item ID 0x4B is Slingshot
            case 0x4C: return "Copy Rod"; // COLLISION FIX: Item ID 0x4C is Copy Rod
