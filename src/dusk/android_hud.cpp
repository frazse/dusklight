...
void hud_update() {
    ...
    // Persistent inventory sync for Quick-Select Grid (using slots 114-161)
    for (int s = 0; s < 24; s++) {
        u8 item = dComIfGs_getItem(s, true);
        if (item != 0xFF) {
            iData[114 + s] = item;
            int count = 1;
            if (item == 0x43 || item == 0x59) count = (int)dComIfGs_getArrowNum();
            else if (item == 0x4B) count = (int)dComIfGs_getPachinkoNum();
            else if (s >= 15 && s <= 17) count = (int)dComIfGs_getBombNum(s - 15);
            else if (s >= 11 && s <= 14) count = (int)dComIfGs_getBottleNum(s - 11);
            iData[138 + s] = count;
    ...
    jintArray jInts = env->NewIntArray(170); env->SetIntArrayRegion(jInts, 0, 170, iData); jfloatArray jF = env->NewFloatArray(14); env->SetFloatArrayRegion(jF, 0, 14, fData);
